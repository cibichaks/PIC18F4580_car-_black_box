...
void putch(unsigned char byte) 
{
	...
	while(!TXIF)// here TXIF is 0 means transmission is happening so whenever its become 1 we can transfer the data
	{
		continue;
	}
	TXIF = 0;
	TXREG = byte;///using this TXREG reg we can transfer byte  
} 
...
int puts(const char *s)
{
	while(*s)		
	{
		putch(*s++); /// here we trafer the string by byte by byte	
	}
	return 0;
}
...
unsigned char getch(void)
{
	...
	RCIF = 0;
	return RCREG;// it has the recived the value from the sender so it will return to the function call
}
...
unsigned char getche(void)
{
	...
	putch(c = getch());// the previous getche is calling here for  
